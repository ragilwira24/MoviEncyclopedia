...
public class TopRatedService {
    ...
    public static void topRatedItems(View view, RecyclerView recyclerView, TopRatedAdapter adapter, List<Result> results, FragmentActivity fragmentActivity){
        recyclerView = view.findViewById(R.id.topRatedRV);
        RecyclerView.LayoutManager layoutManager = new LinearLayoutManager(fragmentActivity, LinearLayoutManager.VERTICAL, false);
        recyclerView.setLayoutManager(layoutManager);
        recyclerView.setItemAnimator(new DefaultItemAnimator());
        recyclerView.setAdapter(adapter);
        recyclerView.setNestedScrollingEnabled(false);
        fetchTopRatedItems(results, adapter, view);
    }

    private static void fetchTopRatedItems(final List<Result> results, final TopRatedAdapter adapter, View view){

        final RelativeLayout relativeLayout = view.findViewById(R.id.topRatedSectionPB);

        String url = MoviesUrlConstant.getTopRatedUrl() + "?api_key="+ CommonConstant.getApiKey() + "&language=en-US";
